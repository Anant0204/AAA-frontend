        ...
        <DialogContent sx={{ p: 0, bgcolor: '#f8fafc' }}>
          {loadingCommHistory ? (
            ...
            <Box sx={{ p: 5, textAlign: 'center' }}>
              ...
              <Typography variant="body2" color="text.disabled" sx={{ mt: 0.5 }}>Rate changes will appear here once a rate is modified.</Typography>
            </Box>
          ) : (
                  ...
                  <Paper key={entry.id} elevation={0} sx={{ border: '1px solid', borderColor: isIncrease ? 'success.light' : 'warning.light', borderLeft: '4px solid', borderLeftColor: isIncrease ? 'success.main' : 'warning.main', borderRadius: 3, p: 2.5, bgcolor: 'white' }}>
                    ...
                    <Box sx={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        <Avatar sx={{ width: 28, height: 28, bgcolor: 'primary.main', fontSize: 13 }}>{(entry.changedBy?.fullName || 'A')[0].toUpperCase()}</Avatar>
                        <Box>
