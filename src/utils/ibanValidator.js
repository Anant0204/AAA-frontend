/**
 * Official IBAN Validator Utility (ISO 13616 & ISO 7064 MOD-97) for Frontend
 * 
 ...
 * @returns {string}
 */
export function normalizeIBAN(rawIBAN) {
...
export function validateIBAN(rawIBAN) {
  ...
  // 2. Minimum/maximum length check
  ...
  if (normalized.length !== expectedLength) {
    return {
      ...
      normalizedIBAN: normalized,
      countryCode,
      error: `Invalid length for ${countryCode} (expected ${expectedLength}, got ${normalized.length})`
    };
  }

  // 5. Check digits must be numbers between 02 and 98
...
/**
 * Masks an IBAN for safe display without exposing sensitive full account numbers.
 * Example: "ES9121000418450200051332" -> "ES91 **** **** **** 1332"
 * @param {string} rawIBAN 
 * @returns {string}
 */
export function maskIBAN(rawIBAN) {
  const normalized = normalizeIBAN(rawIBAN);
 ...
 * @returns {string}
 */
export function formatIBAN(rawIBAN) {
  const normalized = normalizeIBAN(rawIBAN);
