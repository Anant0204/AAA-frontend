...
export const dbService = {
  ...
  // ─── Automated Client Journey Helpers ─────────────────────────────────────
  verifyMagicToken: async (token) => {
    const res = await apiClient.post('/auth/verify-magic-token', { token });
    return res.data;
  },
  selectPackage: async (packageData) => {
    const clientId = packageData?.clientId;
    const url = clientId ? `/clients/${clientId}/select-package` : '/clients/select-package';
    const res = await apiClient.post(url, packageData);
    return res.data;
  },
  createStripeCheckout: async (paymentData) => {
    const res = await apiClient.post('/payments/create-checkout-session', paymentData);
    return res.data;
  }
};
